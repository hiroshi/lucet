...
impl<'a> FuncInfo<'a> {
    pub fn new(
        ...
        Self {
            ...
            codegen_context,
            count_instructions,
            scope_costs: vec![(0, false)],
            vmctx_value: None,
            global_base_value: None,
            runtime_funcs: HashMap::new(),
            // variable indices correspond ot Wasm bytecode's index space,
    ...
    fn load_instr_count(&mut self, builder: &mut FunctionBuilder<'_>) {
        ...
        //    let instruction_count: u64 = *instruction_count_ptr;
    ...
    fn update_instruction_count_instrumentation_pre(
        ...
        fn flush_counter(environ: &mut FuncInfo<'_>, builder: &mut FunctionBuilder<'_>) {
            match environ.scope_costs.last() {
                Some((count, _)) if *count == 0 => return,
                _ => {}
            }
        ...
        if reachable {
            ...
            // overcount)
            assert_eq!(self.scope_costs.last().unwrap().0, 0);
        }
        ...
        match op {
            Operator::CallIndirect { .. } | Operator::Call { .. } => {
                ...
                if reachable {
                    ...
                    self.scope_costs.last_mut().map(|(ref mut x, _)| *x = 1);
                }
            }
            Operator::Block { .. } | Operator::Loop { .. } | Operator::If { .. } => {
                ...
                self.scope_costs.push((0, is_loop));
            }
            Operator::End => {
                // close the top scope
